...
def weighted_path_finder(final_dist,from_node,to_node):
   current = to_node
   path = []   
   while current in final_dist.keys():
      path.insert(0,current)
      current = final_dist[current][1]
   return path
...
def maximize_probability_of_favor(G, v1, v2):
    ...
    # for v2
    nodes = len(G.keys())
    edges = len([(a,b) for a in G for b in G if b in G[a].keys()])
    new_graph = {node1:{node2:-math.log(prob) for node2,prob in value.items()} for node1,value in G.items()}
    #nodes**2 >= edges*math.log(nodes):
    final_dist = dijkstra_heap(new_graph,v1) if math.pow(nodes,2) >= edges*math.log(nodes) else dijkstra_list(new_graph,v1)
    path = weighted_path_finder(final_dist,v1,v2)
    i = 0
    prob = 1
    while i+1<len(path):
        prob = prob * G[path[i]][path[i+1]]
        i = i+1
    return path, prob
...
def test():
    ...
    print maximize_probability_of_favor(G, 'a', 'd')
    print path == ['a', 'e', 'f', 'd']
    print abs(prob - .5 * .5 * .5) < 0.001
