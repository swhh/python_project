...
def goal_function(N):
"Return an is_goal function that returns a boolean on a state input"
    goal = N**2/2-1 if N%2==0 else (N-1)*(N/2+1)
    def is_goal(state):
        for pair in state:
            if pair[0]=="*":
                return goal in pair[1]
    return is_goal

def build_board(state,N):
"Build a list representation of board based on state and N size of board"
    board =['.'] * N**2
...
def modify_state((obj,i),state):
"return new state based on action (obj,i)"
    new_state = tuple()
    for (item,locations) in state:
        if item == obj:
            new_pair = (obj,tuple(map(lambda x: x+i,locations)))
            new_state = new_state + (new_pair,)
        else:
            new_state = new_state + ((item,locations),)
    return new_state

def successor_function(N):
"return successor function that returns all permissible actions and states-after-action"
    def successors(state):
        board = build_board(state,N)
        successors_dict = {}
        for obj,locations in ((obj,locations) for obj,locations in state if obj not in '|@'):
            across = True if abs(locations[0]-locations[1]) == 1 else False
            if across:
                i = 1
                while board[locations[-1]+i] in '.@': #go forwards
                    action = (obj,i)
                    new_state = modify_state((obj,i),state)
                    successors_dict[new_state] = action
                    i = i+1
                j = -1
                while board[locations[0]+j] in '.@': #go backwards
                    action = (obj,j)
                    new_state = modify_state((obj,j),state)
                    successors_dict[new_state] = action
                    j = j-1
            else:
                i = 1
                while board[locations[0]-i*N] in '.@': #go upwards
                    action = (obj,-i*N)
                    new_state = modify_state((obj,-i*N),state)
                    successors_dict[new_state] = action
                    i = i+1
                j = 1
                while board[locations[-1]+j*N] in '.@': #go downwards
                    action = (obj,j*N)
                    new_state = modify_state((obj,j*N),state)
                    successors_dict[new_state] = action
                    j = j+1
...
def solve_parking_puzzle(start, N=N):
    ...
    return shortest_path_search(start,successor_function(N),goal_function(N))
...
def locs(start, n, incr=1):
    ...
    location = (start,)
    for i in range(1,n):
        location = location + (start+(i*incr),)
    return location
...
def grid(cars, N=N):
    ...
    goal = ('@',((N**2/2-1),)) if N%2==0 else ('@',((N-1)*(N/2+1),))
    wall_locs = tuple(range(N))
    wall_locs = wall_locs + tuple(sorted(range(N,N**2-N,N) + range(N*2-1,N**2-1,N)))
    wall_locs = tuple([loc for loc in wall_locs if loc not in goal[1]])
    walls = ('|',wall_locs+tuple(range(N**2-N,N**2)))
...
show(puzzle4)
print path_actions(solve_parking_puzzle(puzzle1,N=8))
