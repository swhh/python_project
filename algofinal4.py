...
def old_path_finder(node_predecessor,from_node,to_node):
   current = to_node
   path = []
...
def departure_time(flight):
   depart = float(flight[3].replace(':','.'))
   return depart

def arrival_time(flight):
   arrival = float(flight[4].replace(':','.'))
   return arrival

def connected_flights(flight_details1,flight_details2):
   arrive = float(flight_details1[3].replace(':','.'))
   depart = float(flight_details2[2].replace(':','.'))
   return depart >= arrive

def create_graph(flights):
   G = defaultdict(dict)
   flight_tuples = [(flight[0],flight[1:]) for flight in flights]
   for flight_num,flight_details in flight_tuples:
      for flight_num2,flight_details2 in flight_tuples:
          if flight_details[1]==flight_details2[0] and connected_flights(flight_details,flight_details2):
             G[flight_num][flight_num2] = flight_details[-1]
   return G          


def route_cost(flights,route):
   return reduce(lambda x,y: x+y,map(lambda x: get_flight(flights,x)[-1],route))

    
def route_time(flights,route):
   route_begin = departure_time(get_flight(flights,route[0]))
   route_end = arrival_time(get_flight(flights,route[-1]))
   return route_end - route_begin    
 
def get_flight(flights,flight_num):
   return [flight for flight in flights if flight[0] == flight_num].pop()
   
    
def find_best_flights(flights, origin, destination):
    G = create_graph(flights)
    best_routes = []
    possible_first_flights = [flight[0] for flight in flights if flight[1] == origin]
    possible_last_flights = [flight[0] for flight in flights if flight[2] == destination]
    if possible_last_flights == []:
       return None
    for flight in possible_first_flights:
        final_dist, node_predecessor = dijkstra(G,flight)
        last_flights = set(final_dist.keys()).intersection(set(possible_last_flights))
        if last_flights != set():
           best_route = min([old_path_finder(node_predecessor,flight,last_flight) for last_flight in last_flights],key=lambda x: (route_cost(flights,x),route_time(flights,x)))
           best_routes.append(best_route)
    if best_routes == []:
       return None  
    return min(best_routes,key=lambda x: (route_cost(flights,x),route_time(flights,x)))
...
def test():
    ...
    print flights == [314, 803, 348, 530, 112]
    print flights
    ...
    print flights == [391, 459]
    print find_best_flights(all_flights, 'Wiluna', 'Kalbarri')
